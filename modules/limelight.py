"""
FROM https://github.com/SteelRidgeRobotics/2025Reefscape/blob/main/lib/limelight.py
...
from ntcore import DoubleArrayEntry, NetworkTable, NetworkTableEntry, NetworkTableInstance
...
class PoseEstimate:
    ...
    def __init__(
        ...
        avg_tag_area: float = 0,
        raw_fiducials=None,
        is_megatag_2: bool = False,
    ):
...
class LimelightHelpers:
    ...
    def to_Pose2D(in_data: list[float]) -> Pose2d:
        ...
        if len(in_data) < 6:
            ...
            return Pose2d(Translation2d(in_data[0], in_data[1]), Rotation2d(radians(in_data[5])))
    ...
    def _get_botpose_estimate(limelight_name: str, entry_name: str, is_megatag_2: bool) -> PoseEstimate:
        ...
        raw_fiducials = (
            [
                RawFiducial(int(pose_array[i]), *pose_array[i + 1 : i + 7])
                for i in range(base_index, base_index + tag_count * vals_per_fiducial, vals_per_fiducial)
            ]
            if len(pose_array) >= base_index + tag_count * vals_per_fiducial
            else []
        )
    ...
    def get_limelight_double_array_entry(table_name: str, entry_name: str) -> DoubleArrayEntry:
        ...
        return LimelightHelpers._double_array_entries.setdefault(
            key, LimelightHelpers.get_limelight_NTTable(table_name).getDoubleArrayTopic(entry_name).getEntry([])
        )
    ...
    def set_limelight_NTDoubleArray(table_name: str, entry_name: str, val: list[float]) -> None:
        LimelightHelpers.get_limelight_NTTableEntry(table_name, entry_name).setDoubleArray(val)  # type: ignore[arg-type]
    ...
    def get_tv(limelight_name: str) -> bool:
        ...
        """
        return LimelightHelpers.get_limelight_NTDouble(limelight_name, "tv") == 1.0
    ...
    @staticmethod
    def set_crop_window(
        limelight_name: str, crop_x_min: float, crop_x_max: float, crop_y_min: float, crop_y_max: float
    ) -> None:
        """
    ...
    def set_robot_orientation(
        limelight_name: str, yaw: float, yaw_rate: float, pitch: float, pitch_rate: float, roll: float, roll_rate: float
    ) -> None:
        """
    ...
    @staticmethod
    def set_robot_orientation_no_flush(
        limelight_name: str, yaw: float, yaw_rate: float, pitch: float, pitch_rate: float, roll: float, roll_rate: float
    ) -> None:
        LimelightHelpers._set_robot_orientation(
            limelight_name, yaw, yaw_rate, pitch, pitch_rate, roll, roll_rate, False
        )

    @staticmethod
    def _set_robot_orientation(
        limelight_name: str,
        yaw: float,
        yaw_rate: float,
        pitch: float,
    ...
    def set_fiducial_id_filters_override(limelight_name: str, valid_ids: list[int]) -> None:
        ...
        for i in range(len(valid_ids)):
            valid_ids_float.append(float(valid_ids[i]))
    ...
    @staticmethod
    def set_camerapose_robotspace(
        limelight_name: str, forward: float, side: float, up: float, roll: float, pitch: float, yaw: float
    ) -> None:
        """
